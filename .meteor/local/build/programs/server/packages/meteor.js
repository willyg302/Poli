...
//                                                                                                  // 57
Meteor.bindEnvironment = function (func, onException, _this) {                                      // 58
  if (!Fiber.current)                                                                               // 59
    throw new Error(noFiberMessage);                                                                // 60
                                                                                                    // 61
  var boundValues = _.clone(Fiber.current._meteor_dynamics || []);                                  // 62
                                                                                                    // 63
  if (!onException)                                                                                 // 64
    throw new Error("onException must be supplied");                                                // 65
                                                                                                    // 66
  return function (/* arguments */) {                                                               // 67
    var args = _.toArray(arguments);                                                                // 68
                                                                                                    // 69
    var runWithEnvironment = function () {                                                          // 70
      var savedValues = Fiber.current._meteor_dynamics;                                             // 71
      try {                                                                                         // 72
        // Need to clone boundValues in case two fibers invoke this                                 // 73
        // function at the same time                                                                // 74
        Fiber.current._meteor_dynamics = _.clone(boundValues);                                      // 75
        var ret = func.apply(_this, args);                                                          // 76
      } catch (e) {                                                                                 // 77
        onException(e);                                                                             // 78
      } finally {                                                                                   // 79
        Fiber.current._meteor_dynamics = savedValues;                                               // 80
      }                                                                                             // 81
      return ret;                                                                                   // 82
    };                                                                                              // 83
                                                                                                    // 84
    if (Fiber.current)                                                                              // 85
      return runWithEnvironment();                                                                  // 86
    Fiber(runWithEnvironment).run();                                                                // 87
  };                                                                                                // 88
};                                                                                                  // 89
                                                                                                    // 90
//////////////////////////////////////////////////////////////////////////////////////////////////////
